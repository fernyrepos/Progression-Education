...
[HarmonyPatch(typeof(BuildFacilityCommandUtility),
    nameof(BuildFacilityCommandUtility.BuildFacilityCommands))]
public static class BuildFacilityCommandUtility_BuildFacilityCommands_Patch
{
    public static IEnumerable<Command> Postfix(IEnumerable<Command> __result, BuildableDef building)
    {
        ...
        {
            if (hiddenFacilities != null
                && command is Designator_Build designator
                && hiddenFacilities.Contains(designator.PlacingDef as ThingDef))
            {
            }
            else
            {
