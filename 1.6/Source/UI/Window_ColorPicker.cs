...
public class Window_ColorPicker : Window
{
    ...
    [
        "title", "colorTextfields_0", "colorTextfields_1", "colorTextfields_2", "colorTextfields_3",
        "colorTextfields_4",
    ];
    ...
    {
        var rectDivider =
            layout.NewRow(ButSize.y, VerticalJustification.Bottom);
    ...
    private void ColorPalette(ref RectDivider layout, ref Color color, Color defaultColor,
        bool showDarklight,
        out float paletteHeight)
    {
        using (new TextBlock(TextAnchor.MiddleLeft))
        {
            ...
            var boxSize = 26;
            var rectDivider3 = rectDivider2.NewRow(boxSize);
            var num2 = 4;
            rectDivider3.Rect.SplitVertically(num2 * (boxSize + 2), out var left,
                out var right);
            RectDivider rectDivider4 = new(left, 195906069,
                new Vector2(10f, 2f));
            Widgets.ColorBox(rectDivider4.NewCol(boxSize), ref color,
                defaultColor);
            Widgets.Label(rectDivider4, "Default".Translate().CapitalizeFirst());
            RectDivider rectDivider5 = new(right, 195906069,
                new Vector2(10f, 2f));
            var defaultDarklight = DarklightUtility.DefaultDarklight;
            ...
            Widgets.ColorSelector(rectDivider2, ref color,
                Dialog_GlowerColorPicker.colors,
                out paletteHeight);
    ...
    {
        rect.SplitVertically((rect.width - 26f) / 2f, out var left,
            out var right);
        RectDivider rectDivider = new(left, 195906069);
        var label = "CurrentColor".Translate().CapitalizeFirst();
        var label2 = "OldColor".Translate().CapitalizeFirst();
        var width = Mathf.Max(100f, label.GetWidthCached(), label2.GetWidthCached());
        ...
        RectDivider rectDivider4 = new(right, 195906069);
    ...
    {
        RectAggregator aggregator =
            new(new Rect(layout.Rect.position, new Vector2(125f, 0f)),
                195906069);
        Widgets.ColorTextfields(ref aggregator, ref color,
            ref textfieldBuffers,
            ref textfieldColorBuffer,
            previousFocusedControlName, "colorTextfields",
            editableColorTextfields,
            visibleColorTextfields);
        size = aggregator.Rect.size;

        Rect hexRect = new(aggregator.Rect.x, aggregator.Rect.yMax + 4, 125,
            32);
        if (Widgets.ButtonText(hexRect, "PE_PasteHex".Translate()))
        {
    ...
    public override void DoWindowContents(Rect inRect)
    {
        ...
        {
            RectDivider layout = new(inRect, 195906069);
            ...
            ColorPalette(ref layout, ref color, defaultColor,
                true, out var paletteHeight);
            ColorTextfields(ref layout, out var size);

            var height = Mathf.Max(paletteHeight, 128f, size.y);
            var rectDivider = layout.NewRow(height);
            rectDivider.NewCol(size.x);
            rectDivider.NewCol(250f, HorizontalJustification.Right);
            Widgets.HSVColorWheel(
                rectDivider.Rect.ContractedBy((rectDivider.Rect.width - 128f) / 2f,
                    (rectDivider.Rect.height - 128f) / 2f), ref color,
                ref hsvColorWheelDragging,
                1f);

            layout.NewRow(10f);
            Rect rect = layout.NewRow(34f);
            Widgets.ColorTemperatureBar(rect, ref color,
                ref colorTemperatureDragging, 1f);
    ...
    private static void HeaderRow(ref RectDivider layout)
    {
        using (new TextBlock(GameFont.Medium))
        {
            ...
            var rectDivider =
                layout.NewRow(Text.CalcHeight(taggedString, layout.Rect.width));
            GUI.SetNextControlName(focusableControlNames[0]);
            var rect = rectDivider.Rect;
            rect.y -= 5f;
    ...
    private static void TabControl()
    {
            ...
            && Event.current.keyCode == KeyCode.Tab)
        {
            ...
            var currentIndex = focusableControlNames.IndexOf(currentFocus);
            if (currentIndex < 0)
            {
    ...
    public static bool TryGetColorFromHex(string hex, out Color color)
    {
        ...
        try
        {
            ...
            var g = int.Parse(hex.Substring(2, 2),
                NumberStyles.HexNumber);
            var b = int.Parse(hex.Substring(4, 2),
                NumberStyles.HexNumber);
            var a = 255;
            if (hex.Length == 8)
            {
                a = int.Parse(hex.Substring(6, 2),
                    NumberStyles.HexNumber);
            }
