...
namespace ProgressionEducation
{
    ...
    public abstract class PawnRoleSelectionWidgetBase_Fixed<RoleType> : IPawnRoleSelectionWidget where RoleType : class, ILordJobRole
    {
        ...
        public PawnRoleSelectionWidgetBase_Fixed(ILordJobCandidatePool candidatePool, ILordJobAssignmentsManager<RoleType> assignments)
        {
        ...
        private bool DoTryAssign(Pawn pawn, IEnumerable<RoleType> roleGroup, bool sendMessage = true, Pawn insertBefore = null, bool doSound = true)
        {
            ...
            {
                if (assignments.TryAssign(pawn, item, out var _, default, insertBefore))
                {
        ...
        private bool TryAssignReplace(Pawn pawn, IEnumerable<RoleType> roleGroup, Pawn replacing)
        {
            if (!SendTryAssignMessages(pawn, roleGroup, isReplacing: true))
            {
                ...
                else if (val != null && assignments.TryAssign(replacing, val, out var _))
                {
                    Notify_AssignmentsChanged();
                }
            }
